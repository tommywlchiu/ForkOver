...
- Streaming with structured outputs: parse the partial JSON incrementally (`partialJson.ts`) and emit each item once its object closes. Zero-price items are never emitted; the final receipt leaves them out and adds a warning naming them (7.4). If the chosen model can't stream structured output, fall back to a single response and report the latency impact at the M2 checkpoint.
